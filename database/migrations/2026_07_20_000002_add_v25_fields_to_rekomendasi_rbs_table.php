    ...
    public function up(): void
    {
        Schema::table('rekomendasi_rbs', function (Blueprint $table) {
            ...
            if (! Schema::hasColumn('rekomendasi_rbs', 'luas_ha_snapshot')) {
                $table->decimal('luas_ha_snapshot', 8, 4)->nullable()->after('jumlah_pokok_snapshot');
            }
            if (! Schema::hasColumn('rekomendasi_rbs', 'sph_snapshot')) {
                $table->unsignedInteger('sph_snapshot')->nullable()->after('luas_ha_snapshot');
            }
            // Field tahap aktif dan sisa tahunan
            if (! Schema::hasColumn('rekomendasi_rbs', 'active_stage')) {
                $table->unsignedTinyInteger('active_stage')->nullable()->after('kcl_aplikasi_saat_ini')
                    ->comment('Tahap aktif saat ini (1 atau 2)');
            }
            if (! Schema::hasColumn('rekomendasi_rbs', 'status_stage')) {
                $table->string('status_stage', 50)->nullable()->after('active_stage')
            ...
            if (! Schema::hasColumn('rekomendasi_rbs', 'urea_sisa_tahunan')) {
                $table->decimal('urea_sisa_tahunan', 10, 2)->nullable()->after('status_stage');
            }
            if (! Schema::hasColumn('rekomendasi_rbs', 'kcl_sisa_tahunan')) {
                $table->decimal('kcl_sisa_tahunan', 10, 2)->nullable()->after('urea_sisa_tahunan');
            }
            if (! Schema::hasColumn('rekomendasi_rbs', 'tanggal_minimum_tahap_berikutnya')) {
                $table->date('tanggal_minimum_tahap_berikutnya')->nullable()->after('kcl_sisa_tahunan');
            }
            if (! Schema::hasColumn('rekomendasi_rbs', 'alasan_tahap')) {
                $table->text('alasan_tahap')->nullable()->after('tanggal_minimum_tahap_berikutnya');
            }
        });
    }
