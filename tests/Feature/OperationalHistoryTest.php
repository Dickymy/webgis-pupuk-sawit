...
/**
 * Test histori operasional (Pahan v2.7 — 4.6).
 *
 * Setiap create/update/cancel realisasi → histori bertambah.
 */
class OperationalHistoryTest extends TestCase
{
    ...
    /** Create realisasi → histori REALISASI_DIBUAT bertambah */
    public function test_create_realization_records_history(): void
    {
        ...
        $this->assertDatabaseHas('rekomendasi_operasional_histories', [
            'rekomendasi_rbs_id' => $this->rekomendasi->id,
            'event_type' => RekomendasiOperasionalHistory::REALISASI_DIBUAT,
        ]);
    }

    /** Update realisasi → histori REALISASI_DIPERBARUI bertambah */
    public function test_update_realization_records_history(): void
    {
        ...
        $this->actingAs($this->admin, 'admin')
            ->put(route('realisasi-pemupukan.update', $realisasi), [
                'tanggal_realisasi' => now()->toDateString(),
                'urea_realisasi_kg' => 150.0,
                'kcl_realisasi_kg' => 200.0,
                'status_realisasi' => 'SEBAGIAN',
            ]);

        $this->assertDatabaseHas('rekomendasi_operasional_histories', [
            'rekomendasi_rbs_id' => $this->rekomendasi->id,
            'event_type' => RekomendasiOperasionalHistory::REALISASI_DIPERBARUI,
        ]);
    }

    /** Cancel realisasi → histori REALISASI_DIBATALKAN bertambah */
    public function test_cancel_realization_records_history(): void
    {
        $realisasi = RealisasiPemupukan::create([
            'rekomendasi_rbs_id' => $this->rekomendasi->id,
            'blok_lahan_id' => $this->blok->id,
        ...
        $this->assertDatabaseHas('rekomendasi_operasional_histories', [
            'rekomendasi_rbs_id' => $this->rekomendasi->id,
            'event_type' => RekomendasiOperasionalHistory::REALISASI_DIBATALKAN,
        ]);
    }
}
