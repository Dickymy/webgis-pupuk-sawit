...
<div class="w-full">
    <div class="bg-white border border-slate-200 shadow-sm rounded-2xl p-6">
        <form method="POST" action="{{ route('anggota.update', $anggota) }}" class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <input type="text" id="no_hp" name="no_hp" value="{{ old('no_hp', $anggota->no_hp) }}" placeholder="08xxxxxxxxxx"
            ...
            <div class="lg:col-span-2">
                ...
                <textarea id="alamat" name="alamat" rows="2"
                    class="w-full px-4 py-3 bg-white border border-slate-300 rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-colors resize-none">{{ old('alamat', $anggota->alamat) }}</textarea>
            </div>
