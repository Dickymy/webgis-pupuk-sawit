...
$rainMode=old('metode_data_hujan',$isEdit?($kondisiLahan->curah_hujan_mm_bulanan!==null?'data_angka':'perkiraan'):'data_angka');
...
<section data-observation-step="3" class="observation-card">
 ...
 <fieldset class="mt-5 rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
  ...
  <div class="mt-4 grid gap-2 sm:grid-cols-3">
   @foreach(['data_angka'=>['Ada data angka','Isi mm/bulan dan sumber'],'perkiraan'=>['Perkiraan kondisi hujan','Pilih keadaan umum di lapangan']] as $value=>$text)<label class="rain-mode flex min-h-20 cursor-pointer gap-3 rounded-xl border p-3 {{ $rainMode===$value?'border-emerald-500 bg-emerald-50':'border-slate-200' }} dark:border-slate-700"><input type="radio" name="metode_data_hujan" value="{{ $value }}" {{ $rainMode===$value?'checked':'' }} class="mt-1"><span><strong class="block text-sm text-slate-800 dark:text-white">{{ $text[0] }}</strong><small class="text-slate-500">{{ $text[1] }}</small></span></label>@endforeach
  </div>
  <div id="rain-number" class="rain-panel {{ $rainMode==='data_angka'?'':'hidden' }} border-sky-200 bg-sky-50 dark:border-sky-800 dark:bg-sky-950/30">
   ...
   <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
    ...
    <div><label for="rain-source" class="field-label">Sumber data</label><select id="rain-source" name="sumber_curah_hujan" class="{{ $control }}"><option value="">Pilih sumber</option>@foreach(['open-meteo'=>'Open-Meteo (perkiraan lokasi)','manual'=>'Catatan kelompok tani','lainnya'=>'Sumber lainnya'] as $value=>$label)<option value="{{ $value }}" {{ (string)$get('sumber_curah_hujan')===$value?'selected':'' }}>{{ $label }}</option>@endforeach</select>@error('sumber_curah_hujan')<p class="field-error">{{ $message }}</p>@enderror</div>
